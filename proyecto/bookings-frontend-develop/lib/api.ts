...
export async function createCustomer(data: CreateCustomerDto): Promise<Customer> {
  ...
  const payload: any = {};
  // If frontend sends a single `name` string, try to split into nombre/apellido
  const fullName = (data as any).nombre || (data as any).name || '';
  if (fullName && typeof fullName === 'string') {
    const parts = fullName.trim().split(/\s+/);
    payload.nombre = parts[0] || '';
    // Use remainder as apellido; if absent, duplicate nombre to satisfy backend validation
    payload.apellido = parts.slice(1).join(' ') || parts[0] || '';
  } else {
    payload.nombre = (data as any).nombre || '';
...
export async function downloadExportExcel(): Promise<void> {
  const token = getToken();

  // Intentamos descargar el Excel desde el backend si existe el endpoint.
  try {
    const res = await fetch(`${API_URL}/appointments/export`, {
      headers: {
        Authorization: token ? `Bearer ${token}` : "",
      },
    });

    if (res.status === 401) throw new Error("No has iniciado sesión.");
    if (res.status === 403) throw new Error("No tienes permisos para exportar.");
    ...
    // Si el servidor responde 404 (endpoint no implementado), realizamos
    // un fallback en cliente: descargamos los appointments y generamos CSV
    if (res.status === 404) {
      // continuar hacia fallback abajo
    } else {
      throw new Error(`Error ${res.status}`);
    }
  } catch (err) {
    // Si el fetch falló por red o CORS, permitimos el fallback a CSV.
    // Continuamos al export CSV a cliente.
  }

  // Fallback cliente: obtener appointments y construir CSV (compatible con Excel)
  const apps = await getAppointments();
  if (!apps || apps.length === 0) throw new Error("No hay reservas para exportar.");
  ...
  const rows = apps.map((a) => [
    a.id,
    (a as any).hora_reserva || "",
    (a as any).date || "",
    (a as any).time || "",
    ((a as any).customerName || (a as any).customerId) ?? "",
    ((a as any).businessName || (a as any).businessId) ?? "",
    (a as any).serviceName || (a as any).serviceId || "",
    (a as any).status || "",
  ]);
